...
import { AppManifest } from "node_modules/@saleor/app-sdk/types";
...
export default createManifestHandler({
	async manifestFactory({ appBaseUrl, request }) {
		 ...
		 * See docs: TODO
		 */
		// appBaseUrl = "http://host.docker.internal:3001";

		console.log({ appBaseUrl, request: request.body });

		const iframeBaseUrl = process.env.APP_IFRAME_BASE_URL ?? appBaseUrl;
		const apiBaseURL = process.env.APP_API_BASE_URL ?? appBaseUrl;

		console.log({ iframeBaseUrl, apiBaseURL });

		const manifest: AppManifest = {
			name: "Saleor Payment App",
